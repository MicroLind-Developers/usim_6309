...
void mc6309::help_adc(Word& x)
{
	Word	m = fetch_word_operand();

	{
		DWord	t = x + m + cc.bit.c;
		cc.bit.c = btst(t, 16);		// Bit 15 carry out
		x = t & 0xffff;
	}
...
void mc6309::help_adcr()
{
    Byte r = fetch_operand();
    int r1 = (r & 0xf0) >> 4;
    int r2 = (r & 0x0f);

    if (r1 <= 7 && r2 <= 7) {
        DWord t = wordrefreg(r2) + wordrefreg(r1) + cc.bit.c;
        cc.bit.c = btst(t, 16);		// Bit 15 carry out
        cc.bit.n = btst(t, 15);
		wordrefreg(r2) = t & 0xffff;
    } else if (r1 >= 8 && r1 <= 15 && r2 >= 8 && r2 <= 15) {
        Word t = byterefreg(r2) + byterefreg(r1) + cc.bit.c;
        cc.bit.c = btst(t, 8);        // Bit 7 carry out
        cc.bit.n = btst(t, 7);
        wordrefreg(r2) = t & 0xff;
    } else {
        printf("ADCR: %02x %02x\n", r1, r2);
        invalid("invalid ADCR operand");
    }
    cc.bit.v ^= cc.bit.c;
    cc.bit.z = !wordrefreg(r2);
    ++cycles;
}

void mc6309::help_add(Byte& x)
{
	Byte	m = fetch_operand();
...
void mc6309::help_addr()
{
    ...
    if (r1 <= 7 && r2 <= 7) {
        DWord t = wordrefreg(r2) + wordrefreg(r1);
        cc.bit.c = btst(t, 16);		// Bit 15 carry out
        cc.bit.n = btst(t, 15);
		wordrefreg(r2) = t & 0xffff;
    } else if (r1 >= 8 && r1 <= 15 && r2 >= 8 && r2 <= 15) {
...
void mc6309::help_andr()
{
    ...
    if (r1 <= 7 && r2 <= 7) {
        ...
        Word t = byterefreg(r2) & byterefreg(r1);
        cc.bit.n = btst(t, 7);
        wordrefreg(r2) = t & 0xff;
    } else {
        printf("ADCR: %02x %02x\n", r1, r2);
...
void mc6309::help_ld(DWord& x)
{
    x = fetch_dword_operand();
    cc.bit.n = btst(x, 31);
    cc.bit.v = 0;
    cc.bit.z = !x;
}

void mc6309::help_lsl(Byte& x)
{
	cc.bit.c = btst(x, 7);
...
void mc6309::help_psh(Byte w, Word& s, Word& u)
{
...
void mc6309::help_pul(Byte w, Word& s, Word& u)
{
