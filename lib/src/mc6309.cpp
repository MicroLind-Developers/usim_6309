...
//
//	(C) E.Lind
//  Based on mc6809.cpp by R.P.Bellis
//
...
mc6309::mc6309() : a(acc.acc_dword.upper.h_word.a), b(acc.acc_dword.upper.h_word.b), e(acc.acc_dword.lower.l_word.e), f(acc.acc_dword.lower.l_word.f), d(acc.acc_dword.upper.d), w(acc.acc_dword.lower.w), q(acc.q)
{
}
...
struct mc6309::Registers mc6309::get_registers()
{
    struct mc6309::Registers r;
    r.acc.q = q;
    r.dp = dp;
    r.x = x;
    r.y = y;
...
void mc6309::fetch_instruction()
{
	...
	switch (ir & 0xf0) {
		...
		case 0x30: case 0x40: case 0x50:
			if (ir < 0x34) {
				mode = indexed;
			} else if (ir < 0x38 || ir == 0x3c || (ir & 0x00f0) == 0x30) {
				mode = immediate;
			} else {
				mode = inherent;
			}
...
void mc6309::execute_instruction()
{
    printf("IR: %04x (%s)\n", ir, insn);
	switch (ir) {
		case 0x3a:
...
// used for ADCR, EXG and TFR instructions
Word& mc6309::wordrefreg(int r)
{
...
Byte& mc6309::byterefreg(int r)
{
	...
	switch (r) {
		...
		case 11: return dp;
        case 12: return zero.bytes.z0;
        case 13: return zero.bytes.z1;
        case 14: return e;
        case 15: return f;
	}
...
Word& mc6309::ix_refreg(Byte post)
{
	static Word no_return = 0;
...
Word mc6309::fetch_word_operand()
{
	switch (mode) {
		case immediate:
...
DWord mc6309::fetch_dword_operand()
{
	switch (mode) {
		case immediate:
...
Word mc6309::fetch_effective_address()
{
	switch (mode) {
		case extended:
			++cycles;
...
Word mc6309::fetch_indexed_operand()
{
...
std::string mc6309::disasm_indexed()
{
...
std::string mc6309::disasm_operand()
{
